...
\newcommand{\la@contentsepbanner}[1]{
	...
	\begin{tikzpicture}[remember picture, overlay, shift = {(0,0)}]
      ...
      (-\la@student@leftmargin,0) -- (\la@student@textwidth+\la@student@rightmargin,0);
		\node (banner name) [
			fill, \currentcolor, rectangle, rounded corners=10, anchor = east,
      minimum width = 3.5cm, minimum height = 20pt
    ] 
			at (\la@student@textwidth,0) {{\bannernamefont \la@bannername}};
		\node [anchor = west, outer sep = -3pt, inner sep = 0pt] (banner title) at (0,-1.5em) {{\boxtitlefont\noindent#1}};
