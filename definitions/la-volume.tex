	...
	\begin{tikzpicture}[remember picture, overlay]
		...
		\node [anchor = south east, inner sep = 0pt, outer sep = 0pt] at 
		    ($(current page.south east) + (-\la@student@spinemargin, \la@student@lowermargin)$) {%
				\includegraphics[width = 100pt]{impa-capa.png}
			};
		\node [
			anchor = south west, inner sep = 0pt, outer sep = 0pt,
			font = \vphantom{Ag}\volumeauthorfont, text width = .55\la@student@textwidth,
			align = left
		] at ($(current page.south west) + (\la@student@foremargin, \la@student@lowermargin)$)
		{Associação Livro Aberto de Matemática};
	\end{tikzpicture}
	\newpage
	\pagecolor{white}
	\pagestyle{empty}
	\teachercol@on\la@teacher@backgroundcolor{white}
}
